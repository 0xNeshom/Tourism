...
const SideBar = () => {
  ...
  const data = [
    ...
    { id: 2, img: iconly1, title: 'نظرات', href: '/comments' },
  ...
  return (
    <div className='w-[310px] flex-shrink-0 shadow-xl top-10 h-screen sticky bg-white  right-[46px]'>
      ...
      <div className='flex flex-col text-base'>
        {data.map((item) => {
          const fullPath =
            item.href === '/dashboard' ? '/crm' : `/crm${item.href}`;

          return (
            <Link href={fullPath} key={item.id}>
              <div
                className={`flex items-center py-4 px-4 gap-5 w-full rounded-l-lg cursor-pointer ${
                  isActive(item.href)
                    ? 'bg-[#D8BFD829] border-l-8 border-purple-900'
                    : ''
                }`}
              ...
              >
                ...
                <span
                  className={`${isActive(item.href) ? 'text-[#7B4794]' : ''}`}
                >
                  {item.title}
                </span>
              </div>
